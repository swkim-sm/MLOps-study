...
/* harmony import */ var _meta_value__WEBPACK_IMPORTED_MODULE_1__ = __webpack_require__(1);
  ...
  return Promise.resolve($.ajax({
    ...
    if (res.message) {
      ...
      res.message.forEach(item => {
        ...
        const linkifiedMessage = escapedMessage.replace(urlRegex, url => `<a href="${url}" target="_blank">${url}</a>`).replaceAll(dateRegex, date => `<time datetime="${date}+00:00">${Object(_datetime_utils__WEBPACK_IMPORTED_MODULE_2__[/* formatDateTime */ "c"])(`${date}+00:00`)}</time>`);
